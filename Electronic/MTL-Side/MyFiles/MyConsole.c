...
void getColorInput(struct Image* pImage);
...
void MyConsole_Task(void)
{
    ...
    // -------------------------------------------------------------------------
    // The following functions are for testing purposes for the MTL
    // They should be removed in the final version
        
    } else if (strcmp(theCmd, "TestColor") == 0)  {
        struct Image* pImage;
        pImage = (struct Image*) malloc(sizeof(struct Image));
        getColorInput(pImage);
        MyCyclone_Write(BASE_DRAW, 0);
    ...
    } else if (strcmp(theCmd, "TestColorRectangle") == 0)  {
        struct Image* pImage;
        pImage = (struct Image*) malloc(sizeof(struct Image));
        getColorInput(pImage);
        
        MyCyclone_Write(BASE_DRAW, 0x01);
        int i,j;
        for(i = 0 ; i<30000 ; i++){
            for(j=0 ; j<1 ; j++){
                MyCyclone_Write(CYCLONE_RED, pImage->color_r);
    ...
    } else if (strcmp(theCmd, "TestColorDrawMode") == 0)  {
        
        char message[50];
        sprintf(message,"Draw value : %x --- \n",MyCyclone_Read(BASE_DRAW));
        MyConsole_SendMsg(message);
        
    } else if (strcmp(theCmd, "TestMap") == 0)  {
        char message[50];
        sprintf(message,"Entering tile mode...\n");
        MyConsole_SendMsg(message);
        MyCyclone_Write(BASE_DRAW, 0x02); // enter tile mode
        
    // End of the testing MTL functions
    // -------------------------------------------------------------------------
    ...
    else if (strcmp(theCmd, "MTLgesture") == 0) {
        char message[30];
        unsigned int lastGesture = 0;
        unsigned int lastlastGesture = 0;
        while(1)
        {
            lastGesture = (unsigned int)MyCyclone_Read(0x07);
            if (lastGesture != lastlastGesture)
            {
                lastlastGesture = lastGesture;
                sprintf(message,"Last gesture is : %x\n",lastGesture);
                MyConsole_SendMsg(message);
            }
        }
